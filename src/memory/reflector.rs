...
impl Reflector {
    ...
    pub fn should_reflect(&self, session_id: &str, message_count: usize) -> bool {
        ...
        let last = self.last_reflection.read().unwrap()
            .get(session_id)
            .copied()
            .unwrap_or(0);
    ...
    pub async fn reflect(&self, session: &SessionHistory) -> Result<Reflection> {
        ...
        let reflection = self.parse_reflection(&response, &session.session_id, session.messages.len())?;
        ...
        let now = SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        self.last_reflection.write().unwrap()
    ...
    fn build_reflection_prompt(&self, messages: &[HistoryMessage]) -> String {
        let mut prompt = String::from(r#"Analyze this conversation and produce a structured reflection.
...
"#);
    ...
    fn parse_reflection(&self, response: &str, session_id: &str, message_count: usize) -> Result<Reflection> {
        ...
        let reflection = Reflection {
            ...
            title: parsed["title"].as_str().unwrap_or("Untitled session").to_string(),
            narrative: parsed["narrative"].as_str().unwrap_or("").to_string(),
            user_goal: parsed["userGoal"].as_str().unwrap_or("Unknown goal").to_string(),
